...
std::cout << name() << "  opb_reg = " << r_opb.read() << std::endl;
	...
	case VCI_GET_CMD:
		if ( p_vci.cmdval.read() ) {
			...
			// (addr - base addr, aka the offset of the target the @ is pointing to) / (B : the amount of bytes in a word)
			// Is a cell the index of the word we are targeting ?
...
void VciGcdCoprocessor<vci_param>::genMoore()
{
	...
	p_vci.rerror = 0;		// by default, no errors
	p_vci.reop   = 1;		// end of pkt is always 1 in response
	...
	case VCI_RSP_STATUS:
		p_vci.cmdack = false;
		p_vci.rspval = true;
		p_vci.rdata  = (r_exe_fsm.read()==EXE_IDLE ? 0 : 1);
		break;
	case VCI_RSP_RESULT:
		p_vci.cmdack = false;
		p_vci.rspval = true;
