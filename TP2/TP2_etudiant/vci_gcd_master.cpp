...
std::cout << name() << "  opb = " << r_opb.read() << std::endl;
	...
	case RANDOM :
		...
		r_opa = rand() + 1;				 // add 1 to avoid having 0 
		r_opb = rand() + 1;				
		r_fsm = CMD_OPA;
		break;
	case CMD_OPA :
...
void VciGcdMaster<vci_param>::genMoore()
{
	...
	case RSP_STATUS :
		p_vci.cmdval	= false;
		p_vci.cmd		= vci_param::CMD_NOP;	// No OPeration, waiting for response
		p_vci.address	= 0;					// unused
		p_vci.wdata		= 0;					// unused
		p_vci.rspack	= true;
		break; 
     } // end switch
} // end genMoore()
